...
from workspace_os.mission import Mission, SPRINT_PATTERN_FILES, InvalidSlugError
...
def cmd_mission_close(args: argparse.Namespace) -> int:
    ...
    try:
        if identifier.isdigit():
            ...
            matches = [m for m in state.list_missions(workspace_id=workspace_id) if m["mission_id"] == mission_id]
            if not matches:
                print(f"error: mission {identifier!r} not found.", file=sys.stderr)
                return 4
            mission = matches[0]
        else:
            matches = [m for m in state.list_missions(workspace_id=workspace_id) if m["slug"] == identifier]
            if not matches:
                print(f"error: mission {identifier!r} not found.", file=sys.stderr)
                return 4
            mission = matches[0]
        ...
        if mission["status"] == "closed":
            ...
            if closed_at is None:
                ...
                ts_str = datetime.fromtimestamp(closed_at, tz=timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")
        ...
        if new_status is None:
            print(f"error: mission {identifier!r} not found.", file=sys.stderr)
            return 4
        closed_row = [m for m in state.list_missions(workspace_id=workspace_id) if m["mission_id"] == mission_id][0]
        closed_at = closed_row.get("closed_at")
        ts_str = (
            datetime.fromtimestamp(closed_at, tz=timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")
            if closed_at is not None else "unknown"
        )
...
def cmd_validate(args: argparse.Namespace) -> int:
    ...
    output_path: Optional[Path] = Path(args.output) if getattr(args, "output", None) else None
    ...
    try:
        verdict = run_validator(
            ws_root, output_path=output_path, timeout=30,
...
def cmd_agent_run(args: argparse.Namespace) -> int:
    ...
    mission_id = None
    if getattr(args, "mission", None):
        missions = [m for m in state.list_missions(workspace_id=workspace_id) if m["slug"] == args.mission]
        if not missions:
            print(f"error: mission {args.mission!r} not found.", file=sys.stderr)
            return 4
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--version", action="version", version=f"workspace-os {__version__}",
    )
    sub = parser.add_subparsers(dest="command", required=False)

    p_init = sub.add_parser("init", help="Initialize workspace-os state",
                            add_help=True)
    ...
    p_mission_new = msub.add_parser("new", help="Create a new mission directory",
                                    add_help=True)
    ...
    p_mission_new.add_argument("--state-root", type=Path, default=None, help="Override .project-state parent")
    p_mission_new.add_argument("--overwrite", action="store_true", help="Replace existing mission directory")
    p_mission_new.set_defaults(func=cmd_mission_new)

    p_mission_list = msub.add_parser("list", help="List missions for this workspace",
                                     add_help=True)
    ...
    p_mission_close = msub.add_parser("close", help="Close a mission by id or slug (idempotent)",
                                      add_help=True)
    ...
    p_validate = sub.add_parser("validate", help="Run the Python validator and parse verdict",
                                add_help=True)
    ...
    p_validate.add_argument("--accept-drift", action="store_true", help="Accept non-forbidden drift for this run")
    p_validate.add_argument("--accept-rationale", default=None, help="Required rationale with --accept-drift")
    p_validate.add_argument("--mission", default=None, help="Record drift ID against this mission slug")
    p_validate.add_argument("--strict", action="store_true", help="R14: hard-fail on any unexpected drift (overrides --accept-drift for non-mandatory categories). Mandatory drift categories are ALWAYS hard-fail regardless.")
    ...
    p_agent_run = asub.add_parser("run", help="Run a shell command and record it",
                                  add_help=True)
    ...
    p_agent_run.add_argument("command", nargs=argparse.REMAINDER, help="Command to run (everything after --)")
...
def _propagate_top_level_options(top: argparse.Namespace, sub: argparse.Namespace) -> argparse.Namespace:
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    for i, arg in enumerate(argv):
        if not arg.startswith("-") and arg in {"init", "mission", "validate", "agent", "-h", "--help"}:
            split_at = i
            break
    if split_at is None:
