...
def check_path_integrity(workspace: Path) -> CheckResult:
    ...
    if missing:
        return _result(
            "path-integrity",
            False,
            f"workspace-index.json: {len(missing)} missing paths",
...
def check_bootstrap_coherence(workspace: Path) -> list[CheckResult]:
    paths = [
        ...
        workspace / "GOVERNANCE" / "BOOTSTRAP.md",
        workspace / "CONTEXT" / "workspace-index.json",
    ]
    return [
        _result(
            "bootstrap",
            path.exists(),
            f"bootstrap: {path.name} resolves" if path.exists() else f"bootstrap: {path} missing",
        )
        for path in paths
    ]
...
def check_authority_uniqueness(workspace: Path) -> list[CheckResult]:
    ...
    for filename in ("EngineeringIdentity.md", "system-graph.md", "AGENT-REGISTRY.md"):
        ...
        message = (
            f"authority: exactly 1 {filename}"
            if count == 1
            else f"authority: {count} {filename} files (expected 1)"
        )
    ...
    for name in ("IDENTITY.md", "ARCHITECTURE.md"):
        ...
        results.append(
            _result(
                "symlink",
                path.exists(),
                f"symlink: {name} -> {target} ({'resolves' if path.exists() else 'broken'})",
            )
        )
    return results
...
def check_identity_drift(workspace: Path) -> CheckResult:
    ...
    audit = workspace / "ARCHIVE" / "audits" / "IDENTITY-AUTHORITY-MAP-ARCHIVED-2026-06-27.md"
    files = (
        list(governance.glob("*.md"))
        + list(context.glob("*.md"))
        + [context / "workspace-index.json"]
    )
    patterns = [
        re.compile(r"\bI am (an?|the) [A-Z][^.]*(engineer|developer|architect|founder)", re.I),
        re.compile(
            r"\bTaras (Polishchuk is|is) (an?|the) [A-Z][^.]*(engineer|developer|architect|founder)",
            re.I,
        ),
        re.compile(r"engineering identity:\s+\S", re.I),
        re.compile(r"\bcore competency:\s+\S", re.I),
        re.compile(r"\bI am Taras Polishchuk", re.I),
    ]
    hits = []
    for path in files:
    ...
    message = (
        "drift: no identity-statement patterns in canonical (non-audit) governance/CONTEXT"
        if not hits
        else f"drift: {len(hits)} identity-statement patterns in canonical files (redefine-id risk)"
    )
...
def check_amendments(workspace: Path) -> CheckResult:
    ...
    return _result(
        "amendments",
        ok,
        f"amendments: GOVERNANCE/AMENDMENTS.md {'exists (Article X)' if ok else 'missing (Article X)'}",
    )
...
def check_release_policy(workspace: Path) -> CheckResult:
    ...
    return _result(
        "release-policy",
        ok,
        f"release-policy: GOVERNANCE/RELEASE-POLICY.md {'exists' if ok else 'missing'}",
    )
        ...
        for line in doc.read_text(errors="replace").splitlines():
            ...
            for match in re.finditer(r"`(/home/taras/projects/[^`]+)`", line):
                ...
                if not Path(value).exists() and Path(value).name not in historical:
                    missing.append(value)
    message = (
        "governance: every load-bearing path reference in non-audit docs resolves"
        if not missing
        else f"governance: {len(missing)} load-bearing path references do not resolve"
    )
    return _result(
        "governance-references", not missing, message, (f"MISSING: {p}" for p in missing)
    )
...
def check_git_identity_overrides(workspace: Path) -> CheckResult:
    ...
    patterns = [
        (re.compile(r"git\s+-c\s+user\.name\b"), "git -c user.name"),
        (re.compile(r"git\s+-c\s+user\.email\b"), "git -c user.email"),
        (re.compile(r"\bgit\s+config\s+user\.name\b"), "git config user.name"),
        (re.compile(r"\bgit\s+config\s+user\.email\b"), "git config user.email"),
        (re.compile(r"\bGIT_AUTHOR_NAME\b"), "GIT_AUTHOR_NAME"),
        (re.compile(r"\bGIT_AUTHOR_EMAIL\b"), "GIT_AUTHOR_EMAIL"),
        (re.compile(r"\bGIT_COMMITTER_NAME\b"), "GIT_COMMITTER_NAME"),
        (re.compile(r"\bGIT_COMMITTER_EMAIL\b"), "GIT_COMMITTER_EMAIL"),
    ]
    hits = []
    for root in roots:
        for path in _walk(root) if root.exists() else ():
            if (
                ...
                or path.name == "validate-workspace.sh"
            ):
                continue
            try:
            ...
            for lineno, line in enumerate(lines, 1):
                ...
                for pattern, label in patterns:
                    ...
                    if match and not (match.start() > 0 and line[match.start() - 1] == "`"):
                        ...
                        break
    message = (
        "git-identity: no override patterns in reusable automation artifacts (Article XI)"
        if not hits
        else f"git-identity: {len(hits)} override-pattern hits in reusable automation artifacts (Article XI violation)"
    )
...
def check_project_state_root(workspace: Path) -> CheckResult:
    ...
    message = (
        f"project-state-root: only canonical root {canonical}/ exists"
        if not stray
        else f"project-state-root: {len(stray)} stray .project-state directory(ies) outside canonical root"
    )
                ...
                if not (mission / required).is_file():
                    failures.append(
                        _result(
                            ...
                            False,
                            f"mission-state-integrity: {mission} missing {required}",
                        )
                    )
            ...
            if source.is_file() and source.stat().st_size == 0:
                failures.append(
                    _result(
                        "mission-state-integrity",
                        False,
                        f"mission-state-integrity: {source} is empty (no intent declared)",
                    )
                )
            if source.is_file() and "RECORD OF RECONSTRUCTION" in source.read_text(
                errors="replace"
            ):
                decisions = mission / "decisions.md"
                if not decisions.is_file() or "Reconstruction rationale" not in decisions.read_text(
                    errors="replace"
                ):
                    failures.append(
                        _result(
                            "mission-state-integrity",
                            False,
                            f"mission-state-integrity: {mission} has RECORD OF RECONSTRUCTION but no Reconstruction rationale in decisions.md",
                        )
                    )
    return failures or [
        _result(
            "mission-state-integrity",
            True,
            "mission-state-integrity: all sprint dirs have required structural artifacts",
        )
    ]
